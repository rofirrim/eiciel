    ...
    , _b_remove_attribute(Gtk::Stock::REMOVE)
{
    ...
    _xattr_listview.append_column_editable(_("Value"),
        _xattr_list_model._attribute_value);
    ...
    _xattr_listview_container.set_policy(Gtk::POLICY_AUTOMATIC,
...
void EicielXAttrWindow::set_name_edited_attribute(const Glib::ustring& path,
    const Glib::ustring& value)
{
    ...
    if (iter) {
        ...
        try {
            _controller->update_attribute_name(row[_xattr_list_model._attribute_name],
                value);
            ...
            if (toplevel == NULL || !toplevel->get_is_toplevel()) {
                Gtk::MessageDialog renameXAttr(s, false, Gtk::MESSAGE_ERROR,
                    Gtk::BUTTONS_OK);
                renameXAttr.run();
            } else {
                Gtk::MessageDialog renameXAttr(*(Gtk::Window*)toplevel, s, false,
...
void EicielXAttrWindow::set_value_edited_attribute(const Glib::ustring& path,
    const Glib::ustring& value)
{
    Gtk::TreeModel::iterator iter = _ref_xattr_list->get_iter(path);
    ...
    if (iter) {
        Gtk::TreeModel::Row row(*iter);
        try {
            _controller->update_attribute_value(
                row[_xattr_list_model._attribute_name], value);
            ...
            Gtk::Container* toplevel = this->get_toplevel();
            if (toplevel == NULL || !toplevel->get_is_toplevel()) {
                Gtk::MessageDialog editXAttr(s, false, Gtk::MESSAGE_ERROR,
                    Gtk::BUTTONS_OK);
                editXAttr.run();
            } else {
                Gtk::MessageDialog editXAttr(*(Gtk::Window*)toplevel, s, false,
                    Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK);
                editXAttr.run();
            }
        }
...
void EicielXAttrWindow::remove_selected_attribute()
{
    ...
    if (iter) {
        ...
        try {
            ...
            Gtk::Container* toplevel = this->get_toplevel();
            if (toplevel == NULL || !toplevel->get_is_toplevel()) {
                Gtk::MessageDialog removeXAttr(s, false, Gtk::MESSAGE_ERROR,
                    Gtk::BUTTONS_OK);
                removeXAttr.run();
            } else {
                Gtk::MessageDialog removeXAttr(*(Gtk::Window*)toplevel, s, false,
                    Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK);
                removeXAttr.run();
            }
        }
...
void EicielXAttrWindow::add_selected_attribute()
{
    ...
    try {
        ...
        Gtk::Container* toplevel = this->get_toplevel();
        if (toplevel == NULL || !toplevel->get_is_toplevel()) {
            Gtk::MessageDialog addXAttr_message(s, false, Gtk::MESSAGE_ERROR,
                Gtk::BUTTONS_OK);
            addXAttr_message.run();
        } else {
            Gtk::MessageDialog addXAttr_message(*(Gtk::Window*)toplevel, s, false,
                Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK);
            addXAttr_message.run();
        }
    }
