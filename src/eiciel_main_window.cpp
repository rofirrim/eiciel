    ...
    , _controller(cont) // Assign _controller to _window
{
    ...
    if (reading_column != NULL) {
        ...
        reading_column->set_cell_data_func(
            *renderRead, mem_fun(*this, &EicielWindow::acl_cell_data_func));
    }
    ...
    if (writing_column != NULL) {
        ...
        writing_column->set_cell_data_func(
            *renderWrite, mem_fun(*this, &EicielWindow::acl_cell_data_func));
    }
    ...
    if (execution_column != NULL) {
        ...
        execution_column->set_cell_data_func(
            *renderExecute, mem_fun(*this, &EicielWindow::acl_cell_data_func));
    }
    ...
    _filter_entry.signal_changed().connect(
        sigc::mem_fun(*this, &EicielWindow::filter_entry_text_changed));
    _filter_entry.signal_icon_press().connect(
        sigc::mem_fun(*this, &EicielWindow::on_clear_icon_pressed));
    ...
    _ref_participants_list_filter->set_visible_func(
        sigc::mem_fun(*this, &EicielWindow::filter_participant_row));
    ...
    _participant_entry.signal_changed().connect(
        sigc::mem_fun(*this, &EicielWindow::participant_entry_box_changed));
    _participant_entry.signal_activate().connect(
        sigc::mem_fun(*this, &EicielWindow::participant_entry_box_activate));
    ...
    _cb_show_system_participants.signal_toggled().connect(
        sigc::mem_fun(*this, &EicielWindow::toggle_system_show));
    ...
    toggle_render->signal_toggled().connect(sigc::bind<PermissionKind>(
        sigc::mem_fun(*this, &EicielWindow::change_permissions), PK_READING));
    ...
    toggle_render->signal_toggled().connect(sigc::bind<PermissionKind>(
        sigc::mem_fun(*this, &EicielWindow::change_permissions), PK_WRITING));
    ...
    toggle_render->signal_toggled().connect(sigc::bind<PermissionKind>(
        sigc::mem_fun(*this, &EicielWindow::change_permissions), PK_EXECUTION));
    ...
    _rb_acl_user.signal_clicked().connect(
        sigc::bind<std::set<std::string>*, ElementKind, Glib::RefPtr<Gdk::Pixbuf>,
            ...
            sigc::mem_fun(*this, &EicielWindow::fill_participants), &_users_list,
            EK_ACL_USER, _user_icon_acl, _default_user_icon_acl));

    _rb_acl_group.signal_clicked().connect(
        sigc::bind<std::set<std::string>*, ElementKind, Glib::RefPtr<Gdk::Pixbuf>,
            Glib::RefPtr<Gdk::Pixbuf>>(
            sigc::mem_fun(*this, &EicielWindow::fill_participants), &_groups_list,
...
bool EicielWindow::acl_selection_function(
...
void EicielWindow::set_value_drag_and_drop(
...
void EicielWindow::get_value_drag_and_drop(
...
void EicielWindow::start_drag_and_drop(
    const Glib::RefPtr<Gdk::DragContext>& context)
{
    Glib::RefPtr<Gtk::TreeSelection> selection_list_reference = _listview_participants.get_selection();
    Gtk::TreeModel::iterator iter = selection_list_reference->get_selected();
...
void EicielWindow::on_clear_icon_pressed(Gtk::EntryIconPosition icon_position,
...
void EicielWindow::filter_entry_text_changed()
{
    ...
    Glib::signal_timeout().connect(sigc::mem_fun(*this, &EicielWindow::refilter),
        filter_delay);
}

bool EicielWindow::filter_participant_row(
